...
const FormEditKategoriVideo = ({ videoCategory, callback }: { videoCategory?: VideoCategory, callback: () => void }) => {
  ...
  useEffect(() => {
    if (videoCategory) {
      setValue('title', videoCategory.title);
      setValue('description', videoCategory.description);
    }
  }, [videoCategory])
  ...
  const onSubmitHandler = async (inputs: VideoCategoryInputTypes) => {
    try {
      if (videoCategory) {
        const data: VideoCategory = {
          ...videoCategory,
          title: inputs.title,
          description: inputs.description
        }
        const response = await VideoRepository.updateVideoCategory({ id: data.id, videoCategory: data });
...
export default FormEditKategoriVideo;