...
import { ProductAddInputTypes, productAddSchema, ProductEditInputTypes, productEditSchema } from "@utils/schema/productSchema";
import { useEffect } from "react";
import { Button, Col, Form, Row } from "react-bootstrap";
import { useForm } from "react-hook-form";
import 'react-quill/dist/quill.snow.css';

const FormEditProduk = ({ callback, product }: { callback: () => void, product : Product }) => {
  const notification = useNotification({ duration: 300 });

  let methods = useForm<ProductEditInputTypes>({
  ...
  const onSubmitHandler = async (inputs: ProductEditInputTypes) => {
    try {
      if (product) {
        const response = await ProductRepository.updateProduct({
          ...
          unit: inputs.unit,
          category: inputs.category as ProductCategory,
          image: inputs.image,
        });
