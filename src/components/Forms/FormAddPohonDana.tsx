...
import { PohonDanaAddInputTypes, pohonDanaAddSchema } from "@utils/schema/pohonDanaSchema";
...
const FormAddPohonDana = ({ callback }: { callback: () => void }) => {
  ...
  const onSubmitHandler = async (inputs: PohonDanaAddInputTypes) => {
    try {
      const response = await PohonDanaRepository.addPohonDana({
        title: inputs.title,
...
export default FormAddPohonDana;