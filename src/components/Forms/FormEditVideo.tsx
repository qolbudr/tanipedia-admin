...
const FormEditVideo = ({ video, callback }: { video: Video, callback: () => void }) => {
  ...
  useEffect(() => {
    if (video) {
      ...
      setValue('categoryId', video.categoryId.toString());
    }
  }, [video])
  ...
  const onSubmitHandler = async (inputs: VideoInputTypes) => {
    try {
      const data: Video = {
        ...
        link: inputs.link,
        categoryId: parseInt(inputs.categoryId),
      }
      const response = await VideoRepository.updateVideo({ id: video.id, video: data });
