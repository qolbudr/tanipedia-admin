...
import { Article, Users, Video, VideoCategory } from '@prisma/client';
...
import FormAddVideo from '@components/Forms/FormAddVideo';
...
function TableVideo({ }: Props) {
      ...
      },
      {
        ...
        cell: (data) => (
          <div className="d-flex gap-1">
            <Button size="sm" onClick={() => editVideo(data.row.original.id)} variant="primary">
              <FeatherIcon name="Edit" size={14} />
            </Button>
            <Button size="sm" onClick={() => deleteVideo(data.row.original.id)} variant="danger">
  ...
  const editVideo = async (videoId: number) => {
    try {
      const response = await VideoRepository.editVideo({ id: videoId })
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const response = await VideoRepository.deleteVideo({ id: videoId });
  ...
  const dataQuery = useQuery(
    ...
    () => VideoRepository.getVideo({ limit: fetchDataOptions.pageSize, offset: fetchDataOptions.pageIndex, search: fetchDataOptions.searchTerm }),
  ...
  return (
    <>
      {video && (
        ...
        >
          <FormEditVideo video={video} callback={() => {
            dataQuery.refetch();
            setIsEdit(false);
          }} />
        </ModalBootstrap>
