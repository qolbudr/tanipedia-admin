...
import { Article, Users, VideoCategory } from '@prisma/client';
...
import FormEditKategoriVideo from '@components/Forms/FormEditKategoriVideo';
import FormAddKategoriVideo from '@components/Forms/FormAddKategoriVideo';
...
function TableKategoriVideo({ }: Props) {
  ...
  const columnsDefs = React.useMemo<ColumnDef<VideoCategory>[]>(
    () => [
      {
        accessorKey: 'id',
        header: 'No',
      ...
      },
      {
        ...
        cell: (data) => (
          <div className="d-flex gap-1">
            <Button size="sm" onClick={() => editVideoCategory(data.row.original.id)} variant="primary">
              <FeatherIcon name="Edit" size={14} />
            </Button>
            <Button size="sm" onClick={() => deleteVideoCategory(data.row.original.id)} variant="danger">
  ...
  const editVideoCategory = async (videoCategoryId: number) => {
    try {
      const response = await VideoRepository.editVideoCategory({ id: videoCategoryId })
  ...
  const deleteVideoCategory = async (videoCategoryId: number) => {
    try {
      Swal.fire({
        icon: "warning",
        ...
        if (result.isConfirmed) {
          const response = await VideoRepository.deleteVideoCategory({ id: videoCategoryId });
  ...
  const dataQuery = useQuery(
    ...
    () => VideoRepository.getVideoCategories({ limit: fetchDataOptions.pageSize, offset: fetchDataOptions.pageIndex, search: fetchDataOptions.searchTerm }),
  ...
  return (
    <>
      {videoCategory && (
        ...
        >
          <FormEditKategoriVideo videoCategory={videoCategory} callback={() => {
            dataQuery.refetch();
            setIsEdit(false);
          }} />
        </ModalBootstrap>
      ...
      <ModalBootstrap
        show={isAdd}
        title="Tambah Kategori Video"
        size="sm"
        close={() => setIsAdd(false) }
        >
          <FormAddKategoriVideo callback={() => {
            dataQuery.refetch();
            setIsAdd(false);
          }}/>
      </ModalBootstrap>
      <Card>
        <Card.Header>
          <div className='d-flex align-items-center justify-content-between'>
            <Card.Title className="mb-0">Tabel Kategori Video</Card.Title>
            <Button onClick={() => setIsAdd(true)}>Tambah Kategori Video</Button>
          </div>
        </Card.Header>
...
export default TableKategoriVideo;
