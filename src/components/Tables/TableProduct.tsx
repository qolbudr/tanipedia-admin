...
import FormAddProduk from '@components/Forms/FormAddProduk';
...
function TableProduct({ }: Props) {
  ...
  const columnsDefs = React.useMemo<ColumnDef<Product>[]>(
    () => [
      {
        accessorKey: 'id',
        header: 'No',
      ...
      },
      {
        ...
        cell: (data) => (
          <div className="d-flex gap-1">
            <Button size="sm" onClick={() => editProduct(data.row.original.id)} variant="primary">
              <FeatherIcon name="Edit" size={14} />
            </Button>
            <Button size="sm" onClick={() => deleteProduct(data.row.original.id)} variant="danger">
  ...
  const editProduct = async (productId: number) => {
    try {
      const response = await ProductRepository.editProduct({ id: productId })
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const response = await ProductRepository.deleteProduct({ id: productId });
  ...
  const dataQuery = useQuery(
    ...
    () => ProductRepository.getProducts({ limit: fetchDataOptions.pageSize, offset: fetchDataOptions.pageIndex, search: fetchDataOptions.searchTerm }),
  ...
  return (
    <>
      {product && (
        ...
        >
          <FormEditProduk product={product} callback={() => {
            dataQuery.refetch();
            setIsEdit(false);
          }} />
        </ModalBootstrap>
      ...
      <Card>
        <Card.Header>
          <div className='d-flex align-items-center justify-content-between'>
            <Card.Title className="mb-0">Tabel Produk</Card.Title>
            <Button onClick={() => setIsAdd(true)}>Tambah Produk</Button>
          </div>
        </Card.Header>
